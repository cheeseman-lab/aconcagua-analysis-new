...
if SELECTED_METHOD in ["cellpose", "all"]:
    try:
        ...
        print(f"Cellpose 3 loaded successfully")
...
if SELECTED_METHOD in ["cellpose4", "all"]:
    try:
        ...
        print(f"Cellpose 4 (CPSAM) loaded successfully")
    except ImportError as e:
        print(f"Warning: Cellpose 4 not available ({e}). Skipping Cellpose 4 benchmark.")

if SELECTED_METHOD in ["stardist", "all"]:
    try:
        from lib.shared.segment_stardist import segment_stardist
        STARDIST_AVAILABLE = True
        print(f"StarDist loaded successfully")
    except ImportError as e:
...
def prepare_test_images():
    ...
    for plate, images in images_by_plate.items():
        if images:
            ...
            print(f"Copying random tile from plate {plate}: {os.path.basename(random_image)}")
    ...
    for img_path in tqdm(image_paths, desc="Processing images"):
            ...
            try:
                ...
                nuclei_cp, cells_cp, counts_df = segment_cellpose(
                    ...
                    cellpose_kwargs=dict(
                        ...
                        nuclei_cellprob_threshold=CELLPOSE_PARAMS["nuclei_cellprob_threshold"],
                        cell_flow_threshold=CELLPOSE_PARAMS["cell_flow_threshold"],
                        cell_cellprob_threshold=CELLPOSE_PARAMS["cell_cellprob_threshold"]
                    ),
                ...
                nuclei_out = OUTPUT_DIR / "cellpose" / f"P-{plate}_W-{well}_T-{tile}_nuclei.tiff"
                cells_out = OUTPUT_DIR / "cellpose" / f"P-{plate}_W-{well}_T-{tile}_cells.tiff"
                segmentation_stats_out = OUTPUT_DIR / "cellpose" / f"P-{plate}_W-{well}_T-{tile}_segmentation_stats.tsv"
        ...
        if STARDIST_AVAILABLE:
            ...
            try:
                ...
                # Save segmentation results
                nuclei_out = OUTPUT_DIR / "stardist" / f"P-{plate}_W-{well}_T-{tile}_nuclei.tiff"
                cells_out = OUTPUT_DIR / "stardist" / f"P-{plate}_W-{well}_T-{tile}_cells.tiff"
                segmentation_stats_out = OUTPUT_DIR / "stardist" / f"P-{plate}_W-{well}_T-{tile}_segmentation_stats.tsv"
            ...
            try:
                ...
                nuclei_cp4, cells_cp4, counts_df = segment_cellpose(
                    ...
                    cellpose_kwargs=dict(
                        ...
                        nuclei_cellprob_threshold=CELLPOSE4_PARAMS["nuclei_cellprob_threshold"],
                        cell_flow_threshold=CELLPOSE4_PARAMS["cell_flow_threshold"],
                        cell_cellprob_threshold=CELLPOSE4_PARAMS["cell_cellprob_threshold"]
                    ),
                ...
                # Save segmentation results
                nuclei_out = OUTPUT_DIR / "cellpose4" / f"P-{plate}_W-{well}_T-{tile}_nuclei.tiff"
                cells_out = OUTPUT_DIR / "cellpose4" / f"P-{plate}_W-{well}_T-{tile}_cells.tiff"
                segmentation_stats_out = OUTPUT_DIR / "cellpose4" / f"P-{plate}_W-{well}_T-{tile}_segmentation_stats.tsv"
...
def generate_benchmark_summary(results_df):
    ...
    ax = sns.barplot(x="method", y="runtime_seconds", data=results_df, errorbar=None, alpha=0.8, palette=method_colors)
    ...
    plt.figure(figsize=(8, 5))
    ax = sns.barplot(x="method", y="memory_mb", data=results_df, errorbar=None, alpha=0.8, palette=method_colors)
    ...
    ax1 = axes[0, 0]
    sns.barplot(x="method", y="initial_nuclei", data=results_df, errorbar=None, alpha=0.8, ax=ax1, palette=method_colors)
    ...
    ax2 = axes[0, 1]
    sns.barplot(x="method", y="initial_cells", data=results_df, errorbar=None, alpha=0.8, ax=ax2, palette=method_colors)
    ...
    ax3 = axes[1, 0]
    sns.barplot(x="method", y="final_cells", data=results_df, errorbar=None, alpha=0.8, ax=ax3, palette=method_colors)
    ...
    ax4 = axes[1, 1]
    sns.barplot(x="method", y="runtime_seconds", data=results_df, errorbar=None, alpha=0.8, ax=ax4, palette=method_colors)
    ...
    sns.barplot(x="method", y="nuclei_retention", data=results_df, errorbar=None, alpha=0.8, ax=ax1, palette=method_colors)
    ...
    sns.barplot(x="method", y="cell_retention", data=results_df, errorbar=None, alpha=0.8, ax=ax2, palette=method_colors)
    ...
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "initial_count_correlation.png", dpi=300, bbox_inches='tight')
    plt.close()
    
    # Create retention by stage plots
    # Calculate the differences for the stacked bars
    results_df['edge_removed_nuclei'] = results_df['initial_nuclei'] - results_df['after_edge_removal_nuclei']
    results_df['edge_removed_cells'] = results_df['initial_cells'] - results_df['after_edge_removal_cells']
    results_df['reconciled_nuclei'] = results_df['after_edge_removal_nuclei'] - results_df['final_nuclei']
    results_df['reconciled_cells'] = results_df['after_edge_removal_cells'] - results_df['final_cells']
    ...
    data = results_df.groupby('method')[['final_nuclei', 'reconciled_nuclei', 'edge_removed_nuclei']].mean()
    ...
    ax1.set_ylabel('Count')
    ax1.set_xlabel('Method')
    ax1.legend(['Final Nuclei', 'Lost in Reconciliation', 'Edge Removed'], 
               frameon=True, framealpha=0.9, edgecolor='lightgray')
    
    # Cells pipeline with cleaner appearance
    data = results_df.groupby('method')[['final_cells', 'reconciled_cells', 'edge_removed_cells']].mean()
    data = data.reindex(sorted(data.index))  # Sort alphabetically
    data.plot(kind='bar', stacked=True, ax=ax2, alpha=0.8, color=stack_colors)
    ax2.set_title('Cell Processing Pipeline')
    ax2.set_ylabel('Count')
    ax2.set_xlabel('Method')
    ax2.legend(['Final Cells', 'Lost in Reconciliation', 'Edge Removed'], 
               frameon=True, framealpha=0.9, edgecolor='lightgray')
    ...
    ax1 = axes[0, 0]
    sns.barplot(x="method", y="edge_removal_nuclei_efficiency", data=results_df, errorbar=None, alpha=0.8, ax=ax1, palette=method_colors)
    add_value_labels(ax1, '%.1f%%')
    ...
    ax2 = axes[0, 1]
    sns.barplot(x="method", y="edge_removal_cells_efficiency", data=results_df, errorbar=None, alpha=0.8, ax=ax2, palette=method_colors)
    add_value_labels(ax2, '%.1f%%')
    ...
    ax3 = axes[1, 0]
    sns.barplot(x="method", y="reconciliation_nuclei_efficiency", data=results_df, errorbar=None, alpha=0.8, ax=ax3, palette=method_colors)
    ...
    ax4 = axes[1, 1]
    sns.barplot(x="method", y="reconciliation_cells_efficiency", data=results_df, errorbar=None, alpha=0.8, ax=ax4, palette=method_colors)
    ...
    for method in results_df["method"].unique():
        ...
        print(f"Initial counts: {method_df['initial_nuclei'].mean():.1f} nuclei, {method_df['initial_cells'].mean():.1f} cells")
        print(f"After edge removal: {method_df['after_edge_removal_nuclei'].mean():.1f} nuclei, {method_df['after_edge_removal_cells'].mean():.1f} cells")
        print(f"Final counts: {method_df['final_nuclei'].mean():.1f} nuclei, {method_df['final_cells'].mean():.1f} cells")
        print(f"Retention rates: {method_df['nuclei_retention'].mean():.1f}% nuclei, {method_df['cell_retention'].mean():.1f}% cells")
        print(f"Edge removal efficiency: {method_df['edge_removal_nuclei_efficiency'].mean():.1f}% nuclei, {method_df['edge_removal_cells_efficiency'].mean():.1f}% cells")
        print(f"Reconciliation efficiency: {method_df['reconciliation_nuclei_efficiency'].mean():.1f}% nuclei, {method_df['reconciliation_cells_efficiency'].mean():.1f}% cells")
