...
def parse_slurm_log(log_path):
    ...
    # Extract timestamps
    timestamps = re.findall(r'\[([A-Za-z]{3} [A-Za-z]{3} +\d+ \d+:\d+:\d+ \d+)\]', content)
...
def extract_quality_metrics():
    ...
    ph_info_path = BRIEFLOW_OUTPUT / "phenotype" / "parquets" / f"P-{PLATE}_W-{WELL}__phenotype_info.parquet"
    sbs_info_path = BRIEFLOW_OUTPUT / "sbs" / "parquets" / f"P-{PLATE}_W-{WELL}__sbs_info.parquet"

    phenotype_cells = len(pd.read_parquet(ph_info_path)) if ph_info_path.exists() else None
    ...
    if fast_merge_path.exists():
        ...
        fast_final = pd.read_parquet(fast_final_path) if fast_final_path.exists() else None

        # Barcode metrics
        cells_with_barcode = fast_final['gene_symbol_0'].notna().sum() if fast_final is not None and 'gene_symbol_0' in fast_final.columns else None
        cells_single_gene = fast_final['mapped_single_gene'].sum() if fast_final is not None and 'mapped_single_gene' in fast_final.columns else None
        ...
        results.append({
            ...
            "barcode_rate": cells_with_barcode / len(fast_final) * 100 if fast_final is not None and cells_with_barcode else None,
            "single_gene_rate": cells_single_gene / len(fast_final) * 100 if fast_final is not None and cells_single_gene else None,
            "phenotype_retention": len(fast_final) / phenotype_cells * 100 if phenotype_cells and fast_final is not None else None,
            "sbs_retention": len(fast_final) / sbs_cells * 100 if sbs_cells and fast_final is not None else None,
        })
    ...
    if stitch_merge_path.exists():
        ...
        stitch_final = pd.read_parquet(stitch_final_path) if stitch_final_path.exists() else None

        # Barcode metrics
        cells_with_barcode = stitch_final['gene_symbol_0'].notna().sum() if stitch_final is not None and 'gene_symbol_0' in stitch_final.columns else None
        cells_single_gene = stitch_final['mapped_single_gene'].sum() if stitch_final is not None and 'mapped_single_gene' in stitch_final.columns else None
        ...
        results.append({
            ...
            "barcode_rate": cells_with_barcode / len(stitch_final) * 100 if stitch_final is not None and cells_with_barcode else None,
            "single_gene_rate": cells_single_gene / len(stitch_final) * 100 if stitch_final is not None and cells_single_gene else None,
            "phenotype_retention": len(stitch_final) / phenotype_cells * 100 if phenotype_cells and stitch_final is not None else None,
            "sbs_retention": len(stitch_final) / sbs_cells * 100 if sbs_cells and stitch_final is not None else None,
        })
...
def plot_runtime_comparison(timing_df, output_path):
    ...
    for approach, y_pos in zip(approaches, y_positions):
        ...
        for _, row in approach_data.iterrows():
            ...
            # Draw bar segment
            ax.barh(y_pos, runtime_min, left=left, height=bar_height,
            ...
            if row['step'] in label_steps and runtime_min > 30:
                ...
                ax.text(left + runtime_min/2, y_pos, f"{step_label}\n({runtime_min:.0f}m)",
                       ha='center', va='center', fontsize=8, color='white', fontweight='bold')

            left += runtime_min

        # Total time label at end
        ax.text(left + 5, y_pos, f"Total: {total_time:.0f} min",
...
def plot_distance_distribution(distances, output_path):
    ...
    for approach, dist in distances.items():
        ax.hist(dist, bins=bins, alpha=0.6, label=f"{approach.capitalize()}",
                density=True, color=COLORS.get(approach, '#7f7f7f'), edgecolor='white', linewidth=0.5)
...
def plot_cell_funnel(quality_df, output_path):
    ...
    for i, approach in enumerate(quality_df["approach"].tolist()):
        ...
        color = COLORS.get(approach, '#7f7f7f')
        bars = ax.bar(x + i*width - width/2, values, width, label=approach.capitalize(),
                     color=color, edgecolor='white', linewidth=0.5)
        # Add value labels
        for bar, val in zip(bars, values):
            if val:
                ax.annotate(f'{val/1e6:.2f}M', xy=(bar.get_x() + bar.get_width()/2, val),
                           xytext=(0, 3), textcoords="offset points", ha='center', va='bottom',
                           fontsize=8, fontweight='bold')

    ax.set_ylabel('Cell Count')
    ax.set_title(f'Cell Counts Through Pipeline')
    ax.set_xticks(x)
...
def plot_tile_match_visualization(output_path):
    ...
    meta = pd.read_parquet(BRIEFLOW_OUTPUT / "preprocess" / "metadata" / "phenotype" / f"P-{PLATE}_W-{WELL}__combined_metadata.parquet")
    tile_pos = meta[['tile', 'x_pos', 'y_pos', 'pixel_size_x', 'pixel_size_y']].drop_duplicates()

    # Load phenotype cells
    ph = pd.read_parquet(BRIEFLOW_OUTPUT / "phenotype" / "parquets" / f"P-{PLATE}_W-{WELL}__phenotype_info.parquet")
    ...
    region = ph[
        (ph['global_j'] > j_center - j_range) & (ph['global_j'] < j_center + j_range) &
        (ph['global_i'] > i_center - i_range) & (ph['global_i'] < i_center + i_range)
    ].copy()
    ...
    for _, row in tile_pos.iterrows():
        ...
        if (t_x + ph_tile_size > x_min and t_x < x_max and
            t_y + ph_tile_size > y_min and t_y < y_max):
            ph_tiles.append((t_x, t_y, ph_tile_size))

    # Load SBS cells and metadata
    sbs_meta = pd.read_parquet(BRIEFLOW_OUTPUT / "preprocess" / "metadata" / "sbs" / f"P-{PLATE}_W-{WELL}__combined_metadata.parquet")
    sbs_tile_pos = sbs_meta[['tile', 'x_pos', 'y_pos', 'pixel_size_x', 'pixel_size_y']].drop_duplicates()

    sbs_cells = pd.read_parquet(BRIEFLOW_OUTPUT / "sbs" / "parquets" / f"P-{PLATE}_W-{WELL}__sbs_info.parquet")
    sbs_cells = sbs_cells.merge(sbs_tile_pos, on='tile')
    sbs_cells['global_j'] = sbs_cells['x_pos'] + sbs_cells['j'] * sbs_cells['pixel_size_x']
    sbs_cells['global_i'] = sbs_cells['y_pos'] + sbs_cells['i'] * sbs_cells['pixel_size_y']
    ...
    sbs_cells['fast_merged'] = sbs_cells['fast_merged'].apply(lambda x: x in fast_sbs_matched)

    if has_stitch:
        stitch_sbs_matched = set(zip(stitch['site'], stitch['cell_1']))
        sbs_cells['stitch_merged'] = list(zip(sbs_cells['tile'], sbs_cells['cell']))
        sbs_cells['stitch_merged'] = sbs_cells['stitch_merged'].apply(lambda x: x in stitch_sbs_matched)

    # Select SBS cells in same region
    sbs_region = sbs_cells[
        (sbs_cells['global_j'] > x_min) & (sbs_cells['global_j'] < x_max) &
        (sbs_cells['global_i'] > y_min) & (sbs_cells['global_i'] < y_max)
    ].copy()
    ...
    for _, row in sbs_tile_pos.iterrows():
        t_x, t_y = row['x_pos'], row['y_pos']
        if (t_x + sbs_tile_size > x_min and t_x < x_max and
            t_y + sbs_tile_size > y_min and t_y < y_max):
    ...
    def draw_tiles(ax, tiles, color='#888888'):
        for t_x, t_y, size in tiles:
            rect = mpatches.Rectangle((t_x, t_y), size, size, linewidth=1,
                                       edgecolor=color, facecolor='none', alpha=0.5, clip_on=True)
            ax.add_patch(rect)
    ...
    ax.scatter(matched['global_j'], matched['global_i'], c='#2ca02c', s=2, alpha=0.5, rasterized=True)
    ...
    ax.scatter(unmatched['global_j'], unmatched['global_i'], c='#d62728', s=2, alpha=0.5, rasterized=True)
    draw_tiles(ax, ph_tiles, '#1f77b4')
    ax.set_xlim(x_min - x_pad, x_max + x_pad)
    ax.set_ylim(y_min - y_pad, y_max + y_pad)
    ...
    ax = axes[1, 0]
    matched = region[region['stitch_merged']] if has_stitch else region[region['fast_merged']]
    ax.scatter(matched['global_j'], matched['global_i'], c='#2ca02c', s=2, alpha=0.5, rasterized=True)
    draw_tiles(ax, ph_tiles, '#1f77b4')
    ax.set_xlim(x_min - x_pad, x_max + x_pad)
    ax.set_ylim(y_min - y_pad, y_max + y_pad)
    ...
    ax = axes[1, 1]
    unmatched = region[~region['stitch_merged']] if has_stitch else region[~region['fast_merged']]
    ax.scatter(unmatched['global_j'], unmatched['global_i'], c='#d62728', s=2, alpha=0.5, rasterized=True)
    draw_tiles(ax, ph_tiles, '#1f77b4')
    ax.set_xlim(x_min - x_pad, x_max + x_pad)
    ax.set_ylim(y_min - y_pad, y_max + y_pad)
    ...
    plt.suptitle(f'Phenotype Cells with Phenotype Tile Boundaries - P-{PLATE}_W-{WELL}', fontsize=14, fontweight='bold')
    plt.tight_layout()
    ...
    # Bottom-left: Stitch matched
    ax = axes[1, 0]
    matched = sbs_region[sbs_region['stitch_merged']] if has_stitch else sbs_region[sbs_region['fast_merged']]
    ax.scatter(matched['global_j'], matched['global_i'], c='#2ca02c', s=2, alpha=0.5, rasterized=True)
    draw_tiles(ax, sbs_tiles, '#ff7f0e')
    ax.set_xlim(x_min - x_pad, x_max + x_pad)
    ax.set_ylim(y_min - y_pad, y_max + y_pad)
    ...
    # Bottom-right: Stitch unmatched
    ax = axes[1, 1]
    unmatched = sbs_region[~sbs_region['stitch_merged']] if has_stitch else sbs_region[~sbs_region['fast_merged']]
    ax.scatter(unmatched['global_j'], unmatched['global_i'], c='#d62728', s=2, alpha=0.5, rasterized=True)
    draw_tiles(ax, sbs_tiles, '#ff7f0e')
    ax.set_xlim(x_min - x_pad, x_max + x_pad)
    ax.set_ylim(y_min - y_pad, y_max + y_pad)
    ...
    plt.suptitle(f'SBS Cells with SBS Tile Boundaries - P-{PLATE}_W-{WELL}', fontsize=14, fontweight='bold')
    plt.tight_layout()
    ...
    def count_covering_tiles(global_j, global_i, tiles_df, tile_size):
        ...
        for _, t in tiles_df.iterrows():
            if (t['x_pos'] <= global_j <= t['x_pos'] + tile_size and
                t['y_pos'] <= global_i <= t['y_pos'] + tile_size):
                count += 1
        return count
    ...
    analysis_sample['n_tiles'] = analysis_sample.apply(
        lambda row: count_covering_tiles(row['global_j'], row['global_i'], tile_pos, ph_tile_size_val),
        axis=1
    )
    ...
    print(f"  Outside overlap (1 tile):  {len(no_overlap):,} ({len(no_overlap)/len(analysis_sample)*100:.1f}%)")
    print(f"  Inside overlap (2+ tiles): {len(in_overlap):,} ({len(in_overlap)/len(analysis_sample)*100:.1f}%)")
    ...
    print(f"  Outside overlap: {fast_no_overlap_matched:,} matched ({fast_no_overlap_matched/len(no_overlap)*100:.1f}%), "
          f"{fast_no_overlap_unmatched:,} unmatched ({fast_no_overlap_unmatched/len(no_overlap)*100:.1f}%)")
    print(f"  Inside overlap:  {fast_in_overlap_matched:,} matched ({fast_in_overlap_matched/len(in_overlap)*100:.1f}%), "
    ...
    if has_stitch:
        ...
        print(f"  Outside overlap: {stitch_no_overlap_matched:,} matched ({stitch_no_overlap_matched/len(no_overlap)*100:.1f}%), "
              f"{stitch_no_overlap_unmatched:,} unmatched ({stitch_no_overlap_unmatched/len(no_overlap)*100:.1f}%)")
        print(f"  Inside overlap:  {stitch_in_overlap_matched:,} matched ({stitch_in_overlap_matched/len(in_overlap)*100:.1f}%), "
    ...
    overlap_data = {
        ...
        'fast_match_rate': [fast_no_overlap_matched/len(no_overlap)*100, fast_in_overlap_matched/len(in_overlap)*100],
    }
    if has_stitch:
        overlap_data['stitch_matched'] = [stitch_no_overlap_matched, stitch_in_overlap_matched]
        overlap_data['stitch_unmatched'] = [stitch_no_overlap_unmatched, stitch_in_overlap_unmatched]
        overlap_data['stitch_match_rate'] = [stitch_no_overlap_matched/len(no_overlap)*100, stitch_in_overlap_matched/len(in_overlap)*100]
...
def plot_memory_requirements(output_path):
    ...
    for step in fast_steps:
        mem = resources.get(step, {}).get('mem_mb', config.get('default-resources', {}).get('mem_mb', 3000))
    ...
    # Plot fast steps
    ax1.barh(np.arange(len(fast_df)), fast_df['memory_gb'].values, width,
             label='Fast', color=fast_color, alpha=0.8)

    # Plot stitch steps (offset)
    ax1.barh(np.arange(len(stitch_df)) + len(fast_df) + 1, stitch_df['memory_gb'].values, width,
             label='Stitch', color=stitch_color, alpha=0.8)
    ...
    ax2.axhline(y=1000, color='red', linestyle='--', alpha=0.7, label='1 TB cluster limit')
...
def main():
    ...
    if not args.plots_only:
        ...
        results_df = quality_df.merge(
            timing_df.groupby('approach').agg({
                'runtime_seconds': 'sum',
                'memory_requested_mb': 'max',
                'status': lambda x: 'success' if all(s == 'success' for s in x) else 'degraded'
            }).reset_index().rename(columns={'runtime_seconds': 'total_runtime_seconds'}),
            on='approach'
        )
