...
def get_feature_mappings():
    ...
    for mask in MASKS:
        for channel in CHANNEL_NAMES:
            ...
            # INTENSITY FEATURES
            intensity_maps = [
                (f"{prefix_emu}_int", f"{prefix_mea}_Intensity_IntegratedIntensity", "high"),
                ...
                (f"{prefix_emu}_int_edge", f"{prefix_mea}_Intensity_IntegratedIntensityEdge", "low"),
                (f"{prefix_emu}_mean_edge", f"{prefix_mea}_Intensity_MeanIntensityEdge", "low"),
                (f"{prefix_emu}_std_edge", f"{prefix_mea}_Intensity_StdIntensityEdge", "low"),
                (f"{prefix_emu}_max_edge", f"{prefix_mea}_Intensity_MaxIntensityEdge", "low"),
                (f"{prefix_emu}_min_edge", f"{prefix_mea}_Intensity_MinIntensityEdge", "low"),
                (f"{prefix_emu}_mass_displacement", f"{prefix_mea}_Intensity_MassDisplacement", "medium"),
                (f"{prefix_emu}_lower_quartile", f"{prefix_mea}_Intensity_LowerQuartileIntensity", "high"),
                (f"{prefix_emu}_median", f"{prefix_mea}_Intensity_MedianIntensity", "high"),
                (f"{prefix_emu}_mad", f"{prefix_mea}_Intensity_MADIntensity", "medium"),
                (f"{prefix_emu}_upper_quartile", f"{prefix_mea}_Intensity_UpperQuartileIntensity", "high"),
            ]
            mappings.setdefault("intensity", []).extend(intensity_maps)

            # LOCATION FEATURES
            location_maps = [
                (f"{prefix_emu}_center_mass_r", f"{prefix_mea}_Location_CenterMassIntensity_Y", "low"),
                (f"{prefix_emu}_center_mass_c", f"{prefix_mea}_Location_CenterMassIntensity_X", "low"),
                (f"{prefix_emu}_max_location_r", f"{prefix_mea}_Location_MaxIntensity_Y", "low"),
                (f"{prefix_emu}_max_location_c", f"{prefix_mea}_Location_MaxIntensity_X", "low"),
            ]
            ...
            for i in range(4):
                radial_maps.extend([
                    (f"{prefix_emu}_frac_at_d_{i}", f"{prefix_mea}_RadialDistribution_FracAtD_{i+1}of4", "low"),
                    (f"{prefix_emu}_mean_frac_{i}", f"{prefix_mea}_RadialDistribution_MeanFrac_{i+1}of4", "low"),
                    (f"{prefix_emu}_radial_cv_{i}", f"{prefix_mea}_RadialDistribution_RadialCV_{i+1}of4", "low"),
                ])
            ...
            for idx, name in enumerate(haralick_names):
                ...
                haralick_maps.append(
                    (f"{prefix_emu}_haralick_5_{idx}", f"{prefix_mea}_{name}_3_00_256", "medium")
                )
            ...
            for i in range(7):
                whu_maps.append(
                    (f"{prefix_emu}_weighted_hu_moments_{i}", f"{prefix_mea}_HuMoment_{i}", "medium")
                )
    ...
    for mask in MASKS:
        ...
        hu_maps = []
        for i in range(7):
            hu_maps.append((f"{mask}_hu_moments_{i}", f"{prefix_mea}_HuMoment_{i}", "high"))
        ...
        zernike_pairs = [
            ...
            (8, 0), (8, 2), (8, 4), (8, 6), (8, 8), (9, 1), (9, 3), (9, 5), (9, 7), (9, 9)
        ]
        zernike_maps = []
...
def generate_correlation_plots(corr_df):
    ...
    cluster_summary = valid_df.groupby("cluster").agg({
    ...
    bars = ax.barh(range(len(clusters)), means, xerr=stds, color=colors,
                   edgecolor='black', linewidth=0.5, capsize=3)

    ax.set_yticks(range(len(clusters)))
    ax.set_yticklabels([f"{c} (n={n})" for c, n in zip(clusters, counts)])
    ax.set_xlabel("Mean Pearson Correlation (r)")
    ax.set_title("Feature Cluster Correlation: CP Emulator vs CP Measure", fontweight='bold')
    ax.axvline(x=0, color='black', linestyle='-', linewidth=0.5)
    ax.axvline(x=0.7, color='green', linestyle='--', linewidth=1, alpha=0.7, label='High (r=0.7)')
    ax.axvline(x=0.4, color='orange', linestyle='--', linewidth=1, alpha=0.7, label='Medium (r=0.4)')
    ax.set_xlim(-0.2, 1.1)
    ...
    plt.tight_layout()
    plt.savefig(corr_output_dir / "cluster_correlation_summary.png", dpi=300, bbox_inches='tight')
    plt.close()
    print(f"  Saved cluster_correlation_summary.png")
    ...
    for expected, color in [("high", "green"), ("medium", "orange"), ("low", "red")]:
        ...
        if len(subset) > 0:
            ax.scatter(subset["expected_numeric"] + np.random.normal(0, 0.02, len(subset)),
                      subset["pearson_r"], alpha=0.5, s=30, c=color, label=f"Expected {expected}")
    ...
    plt.tight_layout()
    plt.savefig(corr_output_dir / "expected_vs_actual_correlation.png", dpi=300, bbox_inches='tight')
    plt.close()
    print(f"  Saved expected_vs_actual_correlation.png")
    ...
    for cluster in valid_df["cluster"].unique():
        ...
        bars = ax.barh(range(len(cluster_data)), cluster_data["pearson_r"].values,
        ...
        ax.set_title(f"{cluster.upper()} Features: CP Emulator vs CP Measure", fontweight='bold')
        ax.axvline(x=0, color='black', linestyle='-', linewidth=0.5)
        ax.set_xlim(-0.2, 1.1)

        plt.tight_layout()
        plt.savefig(corr_output_dir / f"cluster_{cluster}_correlations.png", dpi=300, bbox_inches='tight')
        plt.close()

    print(f"  Saved per-cluster correlation plots")

    # 4. DISTRIBUTION HISTOGRAM
    fig, ax = plt.subplots(figsize=(10, 6))

    ax.hist(valid_df["pearson_r"], bins=30, edgecolor='black', alpha=0.7, color='steelblue')
    ax.axvline(x=valid_df["pearson_r"].mean(), color='red', linestyle='--', linewidth=2,
               label=f'Mean: {valid_df["pearson_r"].mean():.2f}')
    ax.axvline(x=valid_df["pearson_r"].median(), color='orange', linestyle='--', linewidth=2,
    ...
    plt.tight_layout()
    plt.savefig(corr_output_dir / "correlation_distribution.png", dpi=300, bbox_inches='tight')
    plt.close()
    ...
    for expected in ["high", "medium", "low"]:
        subset = valid_df[valid_df["expected_correlation"] == expected]
        if len(subset) > 0:
            print(f"  {expected.upper()}: mean={subset['pearson_r'].mean():.3f}, n={len(subset)}")
...
def generate_scatter_plots(corr_df, measure_df, multi_df, max_features_per_cluster=20):
    ...
    for cluster_name in clusters:
        ...
        cluster_data['base_feature'] = cluster_data['emulator_feature'].apply(get_base_feature)
        ...
        color_map = {base: colors[i] for i, base in enumerate(unique_bases[:max_features_per_cluster])}
        ...
        for _, row in plot_data.iterrows():
            ...
            valid = (x > 0) & (y > 0) & ~np.isnan(x) & ~np.isnan(y) & ~np.isinf(x) & ~np.isinf(y)
            x_valid = x[valid]
            y_valid = y[valid]
            ...
            ax.scatter(x_valid, y_valid, alpha=0.4, s=15, c=[color_map[base]],
        ...
        ax.set_title(f"Log-Scale Correlation: {cluster_name}\n(Mean r = {mean_r:.3f}, n = {n_pairs} feature pairs)",
                    fontweight='bold')
...
def prepare_test_images():
    ...
    for img_path in all_aligned_images:
        ...
        if img_name.startswith('P-'):
            ...
            cytoplasm_path = str(SOURCE_DIR / f"{base_name}__identified_cytoplasms.tiff")

            if all(os.path.exists(p) for p in [nuclei_path, cells_path, cytoplasm_path]):
...
def run_comparison_benchmark(images):
    ...
    from lib.phenotype.extract_phenotype_cp_multichannel import extract_phenotype_cp_multichannel
    ...
    for image_info in tqdm(images, desc="Processing images"):
        ...
        try:
            ...
            features = extract_phenotype_cp_measure(
                data_phenotype=data, nuclei=nuclei, cells=cells,
                cytoplasms=cytoplasms, channel_names=CHANNEL_NAMES
            )

            elapsed = time.time() - start_time
            mem = max(0, get_memory_usage() - start_mem)

            features.to_csv(OUTPUT_DIR / "cp_measure" / f"{base_name}__features.csv", index=False)
            all_stats.append({
                'method': 'cp_measure', 'plate': plate, 'well': well, 'tile': tile,
                'feature_count': len(features.columns), 'row_count': len(features),
                'runtime_seconds': elapsed, 'memory_mb': mem,
        ...
        try:
            ...
            features.to_csv(OUTPUT_DIR / "cp_multichannel" / f"{base_name}__features.csv", index=False)
            all_stats.append({
                'method': 'cp_multichannel', 'plate': plate, 'well': well, 'tile': tile,
                'feature_count': len(features.columns), 'row_count': len(features),
                'runtime_seconds': elapsed, 'memory_mb': mem,
                'timestamp': datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            })
...
def generate_comparison_summary(results_df, run_correlation=True):
    ...
    # Calculate speedup
    cp_measure_time = results_df[results_df["method"] == "cp_measure"]["runtime_seconds"].mean()
    cp_multi_time = results_df[results_df["method"] == "cp_multichannel"]["runtime_seconds"].mean()
    ...
    ax = axes[0]
    method_colors = {"cp_measure": COLORS.get("cp_measure", "#8c564b"),
                     "cp_multichannel": COLORS.get("cp_multichannel", "#e377c2")}
    bars = ax.bar(["CP Measure", "CP Multichannel"],
    ...
    ax2 = axes[1]
    ax2.barh(["Speedup Factor"], [speedup], color='forestgreen', edgecolor='black', height=0.5)
    ax2.axvline(x=1, color='red', linestyle='--', linewidth=2, label='No speedup (1x)')
    ax2.set_xlabel("Speedup (x times faster)", fontsize=12)
    ax2.set_title(f"CP Multichannel is {speedup:.0f}x Faster", fontsize=14, fontweight='bold')
    ax2.set_xlim(0, speedup * 1.15)
    ax2.annotate(f'{speedup:.0f}x', xy=(speedup, 0), xytext=(5, 0),
                textcoords="offset points", ha='left', va='center',
                fontweight='bold', fontsize=16, color='forestgreen')
    ...
    cp_measure_mem = results_df[results_df["method"] == "cp_measure"]["memory_mb"].mean()
    cp_multi_mem = results_df[results_df["method"] == "cp_multichannel"]["memory_mb"].mean()
    ...
    cp_measure_feat = results_df[results_df["method"] == "cp_measure"]["feature_count"].mean()
    cp_multi_feat = results_df[results_df["method"] == "cp_multichannel"]["feature_count"].mean()
    ...
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "feature_count_comparison.png", dpi=300, bbox_inches='tight')
    plt.close()
    ...
    for method in results_df["method"].unique():
        ...
        print(f"\n{method.upper()}")
        print(f"  Runtime: {m['runtime_seconds'].mean():.2f}s (+/- {m['runtime_seconds'].std():.2f})")
    ...
    if run_correlation:
        ...
        if measure_df is not None and multi_df is not None:
            ...
            print(f"Saved correlation results to feature_correlations.csv")
...
def time_function(name, func, args):
    ...
    try:
        ...
        return {"name": name, "status": "SUCCESS", "time_seconds": elapsed,
                "feature_count": feature_count, "error": None}
    except Exception as e:
        elapsed = time.time() - start
        print(f"ERROR: {str(e)[:50]}")
        return {"name": name, "status": "ERROR", "time_seconds": elapsed,
                "feature_count": 0, "error": str(e)}


def run_timing_benchmark():
    """Time individual cp_measure functions."""
    # Import cp_measure functions
    from cp_measure.core.measureobjectsizeshape import get_sizeshape, get_zernike, get_ferret
    ...
    from cp_measure.core.measurecolocalization import (
        get_correlation_pearson, get_correlation_manders_fold,
        get_correlation_rwc, get_correlation_costes, get_correlation_overlap
    )
    ...
    for name, func, args in [
        ...
        ("get_correlation_manders_fold", get_correlation_manders_fold, (ch0, ch1, nuclei)),
    ...
    for _, row in df.sort_values("time_seconds", ascending=False).iterrows():
        status = f"{row['time_seconds']:.2f}s" if row['status'] == 'SUCCESS' else 'ERROR'
