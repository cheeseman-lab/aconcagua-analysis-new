...
OUTPUT_DIR = BENCHMARKS_DIR / "results" / "cluster"
...
def load_brieflow_enrichment(cell_class, k):
    ...
    results_dir = BRIEFLOW_OUTPUT / "DAPI_TUBULIN_GH2AX_PHALLOIDIN" / cell_class / str(k)
    metrics_file = results_dir / "CB-Real__global_metrics.json"
        ...
        for k in K_RANGE:
            ...
            if funk:
                row = {
                    ...
                    'string_f1': funk['metrics'].get('STRING', {}).get('f1_score', np.nan),
                    'string_precision': funk['metrics'].get('STRING', {}).get('precision', np.nan),
                    'string_recall': funk['metrics'].get('STRING', {}).get('recall', np.nan),
                    'corum_enrich': funk['metrics'].get('CORUM', {}).get('proportion_enriched', np.nan),
                    'kegg_enrich': funk['metrics'].get('KEGG', {}).get('proportion_enriched', np.nan),
                }
                results.append(row)
            ...
            if brieflow:
                row = {
                    ...
                    'string_f1': brieflow['metrics'].get('STRING', {}).get('f1_score', np.nan),
                    'string_precision': brieflow['metrics'].get('STRING', {}).get('precision', np.nan),
                    'string_recall': brieflow['metrics'].get('STRING', {}).get('recall', np.nan),
                    'corum_enrich': brieflow['metrics'].get('CORUM', {}).get('proportion_enriched', np.nan),
                    'kegg_enrich': brieflow['metrics'].get('KEGG', {}).get('proportion_enriched', np.nan),
                }
                results.append(row)
...
def build_ideal_comparison_table():
    ...
    for config in IDEAL_CONFIGS:
        ...
        if funk and brieflow:
            row = {
                ...
                'funk_string_f1': funk['metrics'].get('STRING', {}).get('f1_score', np.nan),
                'funk_string_precision': funk['metrics'].get('STRING', {}).get('precision', np.nan),
                'funk_string_recall': funk['metrics'].get('STRING', {}).get('recall', np.nan),
                'funk_corum_enrich': funk['metrics'].get('CORUM', {}).get('proportion_enriched', np.nan),
                'funk_kegg_enrich': funk['metrics'].get('KEGG', {}).get('proportion_enriched', np.nan),

                # Brieflow metrics
                'brieflow_k': config['brieflow_k'],
                'brieflow_num_clusters': brieflow['num_clusters'],
                'brieflow_string_f1': brieflow['metrics'].get('STRING', {}).get('f1_score', np.nan),
                'brieflow_string_precision': brieflow['metrics'].get('STRING', {}).get('precision', np.nan),
                'brieflow_string_recall': brieflow['metrics'].get('STRING', {}).get('recall', np.nan),
                'brieflow_corum_enrich': brieflow['metrics'].get('CORUM', {}).get('proportion_enriched', np.nan),
                'brieflow_kegg_enrich': brieflow['metrics'].get('KEGG', {}).get('proportion_enriched', np.nan),
            }
            results.append(row)
    ...
    df['f1_improvement'] = (df['brieflow_string_f1'] - df['funk_string_f1']) / df['funk_string_f1']
    df['corum_improvement'] = (df['brieflow_corum_enrich'] - df['funk_corum_enrich']) / df['funk_corum_enrich']
    df['kegg_improvement'] = (df['brieflow_kegg_enrich'] - df['funk_kegg_enrich']) / df['funk_kegg_enrich']
...
def plot_ideal_comparison(comparison_df, output_path):
    ...
    for metric, title, ax, show_legend in metrics:
        ...
        bars1 = ax.bar(x - width/2, funk_values, width, label=funk_label,
                      color='#d62728', alpha=0.8)
        bars2 = ax.bar(x + width/2, brieflow_values, width, label=brieflow_label,
        ...
        if metric != 'num_clusters':
            for i in range(len(comparison_df)):
                improvement = ((brieflow_values[i] - funk_values[i]) / funk_values[i]) * 100
    ...
    plt.suptitle('Clustering Method Comparison at Ideal Resolutions',
...
def plot_resolution_sweep(sweep_df, output_path):
        ...
        for col in range(2):
            ...
            for cell_class in cell_classes:
                for method in sweep_df['method'].unique():
                    data = sweep_df[(sweep_df['cell_class'] == cell_class) &
                    ...
                    if len(data) > 0:
                        ...
                        ax.plot(data['k'], data[metric_key] * scale, marker=markers[method],
                               color=colors[method], linestyle=linestyle, linewidth=2,
    ...
    plt.suptitle('Resolution Sweep: Brieflow vs Funk et al. 2022 (k=5-15)',
                 fontsize=14, fontweight='bold', y=0.995)
    plt.tight_layout()
    save_figure(fig, output_path)
    plt.close()
...
def plot_pr_curves(sweep_df, output_path):
    ...
    for cell_class in cell_classes:
        for method in sweep_df['method'].unique():
            data = sweep_df[(sweep_df['cell_class'] == cell_class) &
                           (sweep_df['method'] == method)].sort_values('k')
            if len(data) > 0:
    ...
    for idx, cell_class in enumerate(cell_classes):
        ...
        for method in sweep_df['method'].unique():
            ...
            if len(data) > 0:
                ...
                if len(recall) > 0:
                    ...
                    ax.plot(recall, precision, '-o', color=colors[method],
        ...
        ax.plot([0, max_recall], [0, max_precision], 'k--', alpha=0.3,
                linewidth=1, label=diagonal_label)
    ...
    plt.suptitle('Precision-Recall Curves: STRING Enrichment Across Resolutions',
...
def generate_summary_table(comparison_df, output_path):
    ...
    display_df = pd.DataFrame({
        ...
        'Funk STRING F1': comparison_df['funk_string_f1'].apply(lambda x: f'{x:.4f}'),
        'Funk CORUM %': (comparison_df['funk_corum_enrich'] * 100).apply(lambda x: f'{x:.1f}%'),
        'Funk KEGG %': (comparison_df['funk_kegg_enrich'] * 100).apply(lambda x: f'{x:.1f}%'),
        'Brieflow k': comparison_df['brieflow_k'],
        'Brieflow STRING F1': comparison_df['brieflow_string_f1'].apply(lambda x: f'{x:.4f}'),
        'Brieflow CORUM %': (comparison_df['brieflow_corum_enrich'] * 100).apply(lambda x: f'{x:.1f}%'),
        'Brieflow KEGG %': (comparison_df['brieflow_kegg_enrich'] * 100).apply(lambda x: f'{x:.1f}%'),
        'STRING F1 Δ': comparison_df['f1_improvement'].apply(lambda x: f'{x*100:+.1f}%'),
        'CORUM Δ': comparison_df['corum_improvement'].apply(lambda x: f'{x*100:+.1f}%'),
        'KEGG Δ': comparison_df['kegg_improvement'].apply(lambda x: f'{x*100:+.1f}%'),
    })
